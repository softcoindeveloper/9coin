...
// Copyright (c) 2011-2012 NineCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NineCoinTEST0\r");
            Send(hSocket, "WHO #NineCoinTEST0\r");
        } else {
            // randomly join #NineCoin00-#NineCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #NineCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NineCoin%02d\r", channel_number).c_str());
        }
