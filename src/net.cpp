...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "NineCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"node.org","213.252.244.153"},
    {"node.net","213.252.244.154"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. NineCoin is probably already running."), addrBind.ToString().c_str());
        else
