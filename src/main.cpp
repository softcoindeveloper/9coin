...
uint256 hashGenesisBlock("0x10cdfb18ba23a09328f5c99331ac489a5e7c1a1924c8ff5f3503ff711558f1ed");
...
const string strMessageMagic = "NineCoin Signed Message:\n";
...
{
    // NineCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // NineCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "9coin genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0xda567a5fb40591280e8adff1bba565abc59fe0de0583158232bb7d7c4f433b07"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd8, 0xc5, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NineCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NineCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NineCoinMiner(CWallet *pwallet)
{
    printf("NineCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("NineCoin-miner");
    ...
    try { loop {
        ...
        printf("Running NineCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NineCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NineCoinMiner, pwallet));
}
